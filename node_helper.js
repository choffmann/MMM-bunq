...
module.exports = NodeHelper.create({
  ...
  getSavingAccount: function (iban) {
    fetch(
      `https://api.bunq.com/v1/user/${this.userId}/monetary-account-savings`,
      {
        headers: {
          "User-Agent": "MagicMirror",
          "X-Bunq-Client-Authentication": this.sessionToken
          ...
          accounts.forEach((account) => {
            if (account.MonetaryAccountBank !== undefined) {
              account.MonetaryAccountBank.alias.forEach((alias) => {
                ...
                if (
                  alias.type === "IBAN" &&
                  alias.value === monetaryAccount.iban
                ) {
                  // if showTitle is not defined or showTitle === true
                  if (
                    monetaryAccount.showTitle === undefined ||
                    monetaryAccount.showTitle === true
                  ) {
